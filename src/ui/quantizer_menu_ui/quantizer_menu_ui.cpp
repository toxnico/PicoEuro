...
MenuDisplay *QuantizerMenuUI::buildMenu()
{
    ...
    // Root note:
    auto rootNote = new MenuItem(MENU_ROOT_NOTE);
    rootNote->type = ValueType::Choice;
    rootNote->choicesCount = 12;

    const char *notes[12] = {"C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"};

    for(int i=0;i< 12;i++)
        strcpy(rootNote->choices[i], notes[i]);
...
void QuantizerMenuUI::save()
{
    ...
    s.rootNote = menu->root->findByName(MENU_ROOT_NOTE)->getValueInt();
